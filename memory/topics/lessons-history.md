...
- `tweet-digest` dedup only reliably caught duplicates from the single most-recent day's log, not the full multi-day span it fetches over — the 09-13 run declared a 09-10..09-13 fetch window but only cross-checked the 09-12 log, so 4/5 "new" tweets were actually repeats already reported 09-11. **Settled**: fixed 09-14 and held for 7 consecutive runs through 09-20 — every run now explicitly checks all logs inside its declared fetch window.
