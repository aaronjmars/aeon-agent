...
html = open('/home/runner/tmp-sw-security.html', encoding='utf-8', errors='replace').read()
    ...
    if changed:
        ...
        for k in sorted(changed, key=lambda k: -rows[k]["stars"]):
            ...
            if rows[k]["severity"] != p.get("severity"):
                extra = f" (severity {p.get('severity')} -> {rows[k]['severity']})"
            L.append(row_line(k) + extra)
        L.append("")
if gone:
    L.append(f"_Dropped from board: {', '.join(sorted(gone))}_")
open('/home/runner/tmp-sw-report.md', 'w').write("\n".join(L).rstrip() + "\n")
print("NOTIFY")
