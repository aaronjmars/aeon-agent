...
- **aeon-agent fork sync (`aeon-update`):** PR #220 (window `3b4c5a3..bf33365`, 22 commits) opened 2026-09-04 — **all 6 CI checks green, MERGEABLE/CLEAN**, awaiting operator merge (advances baseline to `bf33365`). 32 files: 12 added (dormant vuln-poc-gate/dev-loop-pr/tests), 16 updated, 1 auto-merge; 5 `eyebrowlock.json` entries refreshed to satisfy the new v0.4.2 verify gate (see Lessons). 10 conflicts pending in `pending_conflicts`: skill-health (#1018, needs eyebrow rescan), workflows+CHANGELOG (GLM_REASONING_EFFORT/PoC-gate/dispatch-id/#1024 scheduler fix), 2 cosmetic package.json, 3 carried (llms.txt/.github/README.md/docs/skill-packs.md). Prior: PR #209 (`8b8d719..3b4c5a3`, 34 commits) **merged 2026-09-01 14:59 UTC** — baseline `3b4c5a3`; held-back skills `rightstack`/`skill-article` landed via in-run eyebrow scan; new skill `cortx-reliability`. 8 conflicts carried in `memory/topics/aeon-update-state.json` `pending_conflicts`: 2 NEW (`scripts/llm-gateway.sh`, `.github/workflows/messages.yml` — operator's emergency GLM pin `b054cb0` vs upstream's tiered GLM_MODEL_* superset; PR body recommends adopting upstream) plus carried `CHANGELOG.md`, `.github/workflows/{aeon,ci-tests}.yml`, `llms.txt`, `.github/README.md`, `docs/skill-packs.md`. Lifetime success rate 43% (3/7) — outage-attributed; keeps the status page 🔴 until clean weekly runs accumulate.
