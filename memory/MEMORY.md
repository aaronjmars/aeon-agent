...
- **aeon-agent fork sync (`aeon-update`):** baseline advanced twice this window — PR #193 (08-24, b1d9079..b7a909a, 83 files) then PR #198 (08-25, b7a909a..8b8d719, 51 files); current baseline `8b8d719`. 10 conflicts carried unresolved in `memory/topics/aeon-update-state.json` `pending_conflicts` (operator-customized files like `aeon.yml`/`ci-tests.yml`/`CHANGELOG.md`/`llms.txt` plus 2 skills held back pending `eyebrow`/catalog regen — see Lessons Learned).
- **Working tree anomaly:** `AGENTS.md` shows as deleted (uncommitted) and `notify`/`notify-jsonrender` are untracked, persisting since at least 2026-08-18 — still present 2026-08-30 (12+ days, ~10 consecutive heartbeat flags, no action taken). Out of heartbeat's checked scope to fix; risk is a later `git add -A`-style step landing the AGENTS.md deletion on main. Needs a human/skill decision: restore, delete-and-commit, or gitignore.
