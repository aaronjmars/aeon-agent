...
- **aeon:** PR #512 open (mcp-server README — repo-actions 06-18 idea #4, shipped 06-20). #501 (A2A README, 06-19) merged. Open contributor PRs: #511/#510 (skill packs), #418 (BEAMR, stalled). Both big undocumented sub-apps (a2a + mcp) now have READMEs. repo-actions 06-18 remaining ideas: #2 (`./new-from-template` README docs), #3 (`./install-from-atrium` README docs), #5 (SHOWCASE.md ecosystem snapshot). Also queued: **CODE_OF_CONDUCT.md** (eligible 2026-06-21 once novelty window clears), then SHA-pin workflows (needs workflows-scoped token).
