...
> **Backport note** — Backported from upstream aeon PR #245 (merged 2026-05-26). The skill body is environment-independent and carried over verbatim; only this note is added. Three adaptation points were checked and required no change: (a) the skill already calls `./notify` with a single positional message argument (aeon-agent's notify contract — not upstream's `-f` flag), (b) the cohort source `memory/topics/fork-cohort-state.json` is already handled with graceful degradation — when it is absent (an aeon-agent operator who hasn't run `fork-cohort` yet) the skill falls back to live `gh api repos/{parent}/forks` classification rather than failing, and (c) all state writes already target the local `memory/topics/fleet-skill-adoption-state.json`. Registered with its natural Sunday 22:00 UTC schedule + `enabled: false` (matching the `fork-skill-gap` / `fleet-state` sibling pattern in this repo), not `workflow_dispatch`, so the Sunday fleet-intelligence stack runs in order once an operator enables it.
