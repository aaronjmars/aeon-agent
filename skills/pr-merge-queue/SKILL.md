---
name: pr-merge-queue
description: Daily survey of open external PRs on aaronjmars/aeon-agent — buckets each PR by touched-file risk tier (FAST_TRACK / INFRA_REVIEW / SKILL_PASS / SKILL_WARN_OR_BLOCK / CORE_REVIEW), runs skill-security-scan on every SKILL.md PR, emits one structured digest so the operator can clear the safest candidates first
var: ""
tags: [dev, community]
---

<!--
Backport of upstream aeon PR #318 (pr-merge-queue, merged 2026-06-02).

Adaptations vs upstream:
1. Default target repo `aaronjmars/aeon` → `aaronjmars/aeon-agent` (in step 1 default
   and every URL template throughout). `${var}` repo-override path preserved verbatim
   so an operator can still aim it at `aaronjmars/aeon` or any other repo.
2. `./notify` call already uses the single-positional-argument style on both sides
   (root `notify` line 3 reads `MSG="$1"`), so no notify-call rewrite was needed.
3. `skill-security-scan/scan.sh` is the same source `pr-skill-triage` (backport PR #68)
   reuses on aeon-agent; the May-18 (PR #186 Bash 3.2 array-emptiness) + May-20
   (PR #197 POSIX-ERE) hardening is intact. No forked scanner patterns.
4. Bucketing rubric and design-notes preserved verbatim — the file-bucket
   precedence is what keeps the skill structurally safe and must not be softened.

20th consecutive same-day-after backport in the chain (May-3 anchor):
operator-scorecard May-3→4, skill-freshness May-4→5, skill-update-check May-8→9,
fork-cohort May-9→10, thread-formatter May-11→12, v4-readiness May-12→13,
product-hunt-launch May-15→17, fork-first-run-alert May-17→18,
fork-skill-gap May-18→19, competitor-launch-radar May-19→20,
contributor-spotlight May-21→23, install-skill-pack+registry May-22→24,
ecosystem-pulse May-24→26, fleet-skill-adoption May-26→27,
sparkleware-catalog May-27→28, pr-skill-triage May-28→29,
fork-health-score May-29→30, spend-monitor May-29→Jun-01,
follow-up-patrol May-29→Jun-02, pr-merge-queue Jun-02→Jun-03.
-->

> **${var}** — Optional. Pass `dry-run` to skip notify (article + state still write). Pass `owner/repo` to override the target repo. Combine with a space (`dry-run owner/repo`) for both. Empty = `aaronjmars/aeon-agent`.

Today is ${today}. The open-PR queue on `aaronjmars/aeon-agent` accumulates external skill PRs, backport PRs from the upstream-sync chain, and bot-authored housekeeping PRs that route through `auto-merge`. `pr-skill-triage` evaluates one inbound skill PR at a time (workflow_dispatch), but no skill answers the operator's actual morning question: *"of the N open PRs right now, which N1 can I merge in one click and which N2 need real review?"*
...
| **`pr-merge-queue`** | **All open PRs on the target repo's queue** | **One operator-facing digest sorted by risk + age — no per-PR comment, no merge action** |
...
- `TARGET_REPO=${REPO_OVERRIDE:-aaronjmars/aeon-agent}`.
...
Send via `./notify "$MSG"` (single positional argument — the heredoc-built message; aeon-agent's `./notify` reads `MSG="$1"` at line 3, matching the positional style this skill uses).
...
- **`memory/watched-repos.md` is read for `## Trusted Authors` only.** The skill does not iterate every watched repo by default because the digest format only renders cleanly for one repo at a time and the operator's morning question ("what's safe to merge on aeon-agent today") is per-repo. Multi-repo support is the `${var}` override path.
