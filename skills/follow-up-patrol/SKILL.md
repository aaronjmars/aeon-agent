...
<!-- Verbatim backport of upstream aaronjmars/aeon skills/follow-up-patrol/SKILL.md (merged 2026-05-29 in PR #272 alongside spend-monitor/narrative-convergence/mcp-pulse/fleet-scorecard). One adaptation vs upstream: `./notify -f file` rewritten to `./notify "$MSG"` because aeon-agent's notify script reads $1 (confirmed at root notify line 3 `MSG="$1"`). All other logic preserved verbatim. -->
...
Build the message inline and send it via `./notify "$MSG"` (single positional argument — aeon-agent's `./notify` reads `$1`):
...
## Backport adaptations vs upstream PR #272

1. **`./notify` call style** — upstream uses `./notify -f .pending-notify-temp/follow-up-patrol-${today}.md` (file flag). Aeon-agent's `./notify` reads `$1` as the message (root `notify` line 3: `MSG="$1"`), so step 6 rewritten as an inline heredoc piped through `./notify "$MSG"`. The behavior is equivalent — same message body, same gating, same one notification per run when CRITICAL or HIGH is non-zero.
2. **Issue tracker graceful skip** — aeon-agent does not currently maintain `memory/issues/INDEX.md`; step 3's "If `memory/issues/INDEX.md` doesn't exist, skip this section" branch is the active path on day-one runs here. When the file is added later (the upstream pattern), step 3 starts populating the `## Open Issues` table automatically.
3. **Follow-up section resolution** — aeon-agent's `memory/MEMORY.md` has `## Next Priorities` (verified at line 107), which is rule 2 of the step 1 resolution order, so the skill picks up the existing section without any local memory-format changes.

No other adaptations. Frontmatter (`schedule: "0 11 * * 2"`) preserved verbatim — Tuesday 11:00 UTC slot is free in aeon-agent's `aeon.yml`.
