...
> **Backport note** — verbatim copy of `skills/fork-health-score/SKILL.md` from upstream `aaronjmars/aeon` (PR #271, merged 2026-05-29). Zero adaptation: (1) `./notify` single-positional-arg style already aligned with upstream per prior backport notes, (2) `gh api` access pattern (no curl + env-var headers) already matches CLAUDE.md sandbox guidance, (3) dependent state files (`memory/topics/fork-cohort-state.json`) seed identically from the `fork-cohort` skill already backported here. On aeon-agent the parent resolves to `aaronjmars/aeon-agent` rather than `aaronjmars/aeon` — the same per-repo measurement the rest of the fleet-stack already performs against this repo's own fork list.
