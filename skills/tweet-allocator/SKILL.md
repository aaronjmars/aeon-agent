...
- `aeonframework` — Aeon project account
...
1. **Collect tweet data (last 24h only).** Read today's `memory/logs/${today}.md` for `## Fetch Tweets` sections. Extract every tweet that was reported — for each one, capture:
   ...
   **Required-token filter (defensive).** Read the `fetch-tweets` entry in `aeon.yml` — its `var` lists the OR-separated tokens that define a valid tweet (e.g. `$AEON OR @aeonframework OR github.com/aaronjmars/aeon`). For each candidate tweet, check whether its `summary` / `text` / URL contains **at least one** of those tokens (case-insensitive substring match). Drop any tweet that contains **none** of them — those are stale pre-filter false positives (e.g. a tweet using the bare word "aeon" without the $ cashtag). Log the drops in the run entry under `**Dropped (no required token):**`. This mirrors the `scripts/filter-xai-tweets.py` guard that `fetch-tweets` applies to the Grok cache, protecting against older log entries written before the filter existed.
...
5. **Allocate the budget.** The daily budget is `${var}` if set, otherwise `10` **USD-equivalent, paid in $AEON on Base**. Always phrase the amount as "$X in $AEON" in every output (notification, report, log) so the reader understands the $-sign refers to USD value, not a cashtag for $AEON.
   - Distribute proportionally by score: each tweet's share = `(tweet_score / total_score) * budget`
   - Round to 2 decimal places ($AEON has 6 decimals, but keep amounts human-readable)
...
6. **Send or report.** Check if `BANKR_SEND_KEY` is set:
   ...
   **If `BANKR_SEND_KEY` IS set (auto-send mode):**
   Send $AEON to each verified handle using the read-write key:
   ```bash
   JOB_ID=$(curl -s -X POST "https://api.bankr.bot/agent/prompt" \
...
7. **Build the allocation report** and save to `articles/tweet-allocator-${today}.md`:
   ...
   **Token:** $AEON | **Budget:** $X.XX in $AEON | **Chain:** Base
   ...
   |------|--------|-------|-------|--------|--------|
   | 1 | x.com/handle | [summary](tweet_url) | XX | $X.XX in $AEON | pending |
   | 2 | x.com/handle | [summary](tweet_url) | XX | $X.XX in $AEON | pending |
   | ... | | | | | |

   **Total allocated:** $X.XX in $AEON (manual send required)
...
8. **Send notification** via `./notify`:
   ...
   *Tweet Rewards — ${today}*
   ...
   2. x.com/handle — $X.XX in $AEON (score: XX)
      [brief summary]
      [View tweet](tweet_url)

   ...

   Total: $X.XX in $AEON allocated to N authors (manual send)
   ```

   IMPORTANT: Do NOT use @handle format in the notification — it pings users on Telegram. Use x.com/handle instead. Always write "$X.XX in $AEON" (not just "$X.XX $AEON") so the USD-vs-cashtag distinction is unambiguous.
...
9. **Log** to `memory/logs/${today}.md`:
   ...
   - **Paid tweets**:
     - x.com/handle — $X.XX in $AEON — tweet_url — PENDING (manual send)
     - x.com/handle — $X.XX in $AEON — tweet_url — PENDING (manual send)
   - **Total distributed**: $X.XX / $X.XX in $AEON
