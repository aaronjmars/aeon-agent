...
5. **Allocate the budget.** The daily budget is `${var}` if set, otherwise `10` (USD, paid in in $AEON on Base).
...
7. **Build the allocation report** and save to `articles/tweet-allocator-${today}.md`:
   ...
   **Token:** $TOKEN | **Budget:** $X.XX $AEON | **Chain:** Base
...
8. **Send notification** via `./notify`:
   ...
   *Tweet Rewards — ${today}*
   ...
   1. x.com/handle — $X.XX $AEON (score: XX)
      [brief summary]
      [View tweet](tweet_url)

   2. x.com/handle — $X.XX $AEON (score: XX)
      [brief summary]
      [View tweet](tweet_url)
...
9. **Log** to `memory/logs/${today}.md`:
   ...
   - **Paid tweets**:
     - x.com/handle — $X.XX $AEON — tweet_url — PENDING (manual send)
     - x.com/handle — $X.XX $AEON — tweet_url — PENDING (manual send)
