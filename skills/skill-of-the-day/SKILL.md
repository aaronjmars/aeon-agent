...
---
<!--
  Verbatim backport of upstream aeon `skills/skill-of-the-day/SKILL.md`
  (Nurstar's upstream aeon PR #341, merged 2026-06-04).

  23rd consecutive same-day-after backport (per memory/MEMORY.md
  "Skills Built" table). The article-call-out from articles/repo-actions-2026-06-06.md
  named this skill as the cleanest unbackported one in the current queue.

  Three adaptations vs upstream:

    (1) `./notify` call style: upstream uses `./notify "$(cat .outputs/skill-of-the-day.md)"`
        which already matches aeon-agent's positional-$1 contract — no
        rewrite needed at step 4 (the same call style mcp-pulse / narrative-convergence
        backports had to convert *into*). Single backport where notify
        wiring is a true no-op.

    (2) Queue seed (`memory/topics/skill-of-the-day.md`) — upstream is
        intended to seed from `ls skills/`. The catalog-fallback rule at
        step 1.3 already reads `skills/` and skips blocklist + 30-day
        Covered entries, so a fresh aeon-agent install picks up natively
        from the aeon-agent skill set (currently ~100 skills) without any
        manual queue priming. The seed file is OPTIONAL — when absent
        the catalog-fallback path is the only path and the skill remains
        operational.

    (3) `gh workflow run aeon.yml -f skill="${PICK}"` at step 5 — upstream
        ships verbatim. Confirmed against aeon-agent's `.github/workflows/aeon.yml`:
        the same `workflow_dispatch` `skill` input exists upstream and here
        (no rename), so the dispatch call passes through unchanged. If the
        upstream `aeon.yml` ever renames the input, this is the line that
        moves with it.

  No frontmatter edits — `schedule:` slot is preserved as `workflow_dispatch`
  per upstream (no scheduled cron in upstream — the operator dispatches
  this skill on a content cadence, not the cron does).

  PR #341 originated from contributor Nurstar (community PR — see upstream
  aeon log). This backport keeps the brand line `The @aeonframework skill
  of the day 🌟` verbatim per upstream's own forking-note: forks that want
  a different handle edit only that one string in this file.
-->
...
3. If the queue is exhausted (or the file does not exist on a fresh install), `ls skills/` and pick the first skill that has a `SKILL.md`, is not in the blocklist, and has not been covered in the last 30 days.
...
aeon-agent's `./notify` reads its first positional arg as the message (CLAUDE.md + dynamically-generated workflow notify script: `MSG="$1"`) — this call style matches both upstream aeon and aeon-agent without translation.
...
Edit `memory/topics/skill-of-the-day.md`. If the file does not exist, create it with the canonical three-section layout (`## Queue`, `## Covered (last 30 days)`, `## Blocklist`) populated with empty bodies — the next run reads the file, finds the queue empty, and naturally hits the catalog-fallback at step 1.3 until the operator seeds it.
...
`gh workflow run` works in the sandbox via the gh CLI (auth handled internally). `./notify` fans out to every configured channel and accepts `$1` directly — no prefetch needed.
