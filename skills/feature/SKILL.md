...
- Fetch the canonical upstream text with **WebFetch** (e.g. the Contributor Covenant: `https://www.contributor-covenant.org/version/2/1/code_of_conduct/code_of_conduct.md`) and write it verbatim, customizing only the enforcement-contact field (pull the contact convention from the repo's existing `SECURITY.md`/`CONTRIBUTING.md`).
