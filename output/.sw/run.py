...
html = open('output/.sw/security.html', encoding='utf-8', errors='replace').read()
...
def row_line(k):
    ...
    url = r['fix_url']
    sev = r['severity']
    stars_str = format(r['stars'], ",")
    note = r['note']
    return "- **[" + link_text + "](" + url + ")** — `" + sev + "` · " + stars_str + "★ — " + note
...
if full:
    ...
    if new:
        L.append("### \U0001f195 Newly secured (" + str(len(new)) + ")")
...
open('output/.sw/report.md', 'w').write("\n".join(L).rstrip() + "\n")
print("NOTIFY")
