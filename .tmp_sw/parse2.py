...
L.append(f"## {title}")
L.append(f"**{len(rows)} repos** secured ({sd(d_repos)}) . **{total_stars:,}stars** total ({sd(d_stars)})")
L.append("")
if full:
    ...
    if new:
        L.append(f"### NEW Newly secured ({len(new)})")
        for k in sorted(new, key=lambda k: -rows[k]["stars"]):
            L.append(row_line(k))
        L.append("")
    if changed:
        L.append(f"### UPDATED Updated fix / severity ({len(changed)})")
...
open('.tmp_sw/report2.md', 'w').write("\n".join(L).rstrip() + "\n")
print("NOTIFY")
